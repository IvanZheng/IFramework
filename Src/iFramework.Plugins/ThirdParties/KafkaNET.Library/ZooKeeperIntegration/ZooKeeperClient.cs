...
namespace Kafka.Client.ZooKeeperIntegration
{
    ...
    public partial class ZooKeeperClient : IZooKeeperClient
    {
        ...
        {
            return this.connection == null ? KeeperState.Disconnected : this.connection.ClientState;
        }
        ...
        public void Disconnect()
        {
            ...
            try
            {
                if (this.eventWorker.ThreadState != System.Threading.ThreadState.Unstarted)
                {
        ...
        public bool WaitUntilConnected(int connectionTimeout)
        {
            ...
            {
                this.currentState = this.connection.ClientState;
                if (this.currentState == KeeperState.SyncConnected)
                    return true;
                Logger.DebugFormat("Current state:{0} in the lib:{1}", this.currentState, this.connection.GetInternalZKClient().State);
        ...
        public T RetryUntilConnected<T>(Func<T> callback)
        {
            ...
            throw KeeperException.Create(KeeperException.Code.OPERATIONTIMEOUT);// KeeperException.OperationTimeoutException();
        }
        ...
        public long GetCreateTime(string path)
        {
            ...
            this.EnsuresNotDisposed();
            return this.RetryUntilConnected(
                () => this.connection.GetCreateTime(path));
        }
