...
namespace Kafka.Client.Cfg
{
    ...
    public class KafkaSimpleManagerConfiguration
    {
        ...
        {
            set => ZookeeperConfig = KafkaClientHelperUtils.ToZookeeperConfig(value);
        }
        ...
        ///     Else if this value is one partitioner class full name, the kafkaSimpleManager will only create one producer.  And
        ...
        ///     fetch.wait.max.ms, 100ms, The maximum amount of time the server will block before answering the fetch request if
        ///     there isn't sufficient data to immediately satisfy fetch.min.bytes
