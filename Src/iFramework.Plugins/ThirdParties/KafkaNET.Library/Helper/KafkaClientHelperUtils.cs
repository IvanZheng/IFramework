...
namespace Kafka.Client.Consumers
{
    public class KafkaClientHelperUtils
    {
        private static readonly ILogger Logger =
            IoCFactory.Resolve<ILoggerFactory>().Create(typeof(KafkaClientHelperUtils));
        ...
        public static SyncProducer TryCreateSyncProducer(int brokerId, string host, int port)
        {
            ...
            {
                syncProducer =
                    new SyncProducer(new SyncProducerConfiguration {BrokerId = brokerId, Host = host, Port = port});
            }
            catch (Exception e)
            {
                Logger.Error(
